...
if [ $format = "ssf" ];then
	python extract_features_for_chunk_from_pos_annotated_ssf_files.py --input $input_folder --output $chunk_features_folder
else
...
python correct_incorrect_chunks.py --input $pred_chunks_folder --output $corrected_chunks_folder
output_folder=$input_folder"-Chunk-"$format
if [ $format = "ssf" ];then
	if [ ! -d $output_folder ];then
		mkdir $output_folder
	fi
	python read_feature_files_and_convert_into_ssf.py --input $corrected_chunks_folder --output $output_folder --opr 1
else
